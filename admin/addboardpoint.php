...
{$phpVar =  $_COOKIE['myJavascriptVar'];	/* cookie is used to get the value of route number selected in route column of form */
  $search_result = mysqli_query($conn, "SELECT * FROM bus_master where Route_no = '".$phpVar."' order by Time");
...
?>  
  ...
  <div class="form-group">
    ...
    <input type="text"  class="form-control" id="boardingpoint" name="boardingpoint" placeholder="Enter Boarding Point" size="30" maxlength="50"  required>
  </div>
  </div></div>
...
<script>
  ...
  $('#boardingpoint').on('keypress', function (event) {
...
<div class= "col-lg-6">
  <div class="form-group">
    <label>Fee</label><br>
    <input type="text"  class="form-control" id="fee" name="fee" placeholder="Enter the fee details" size="30" maxlength="6" required>
...
<script>
  ...
  $('#fee').on('keypress', function (event) {
